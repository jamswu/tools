...
try:
    ...
    with open("config/config.cfg") as cfgfile:
        ...
        table_struc_dir=config.get("backmysql","table_struc_dir")
        ...
        import_table_struc=config.getboolean("backmysql","import_table_struc")
...
try:
    ...
    table_struc = " ".join(list(set(all_table) - set(full_table)))
    cur.close()
    conn.close()
except MySQLdb.Error,e:
    print "Mysql Error %d: %s" % (e.args[0],e.args[1])
    sys.exit()
os.system("mydumper -B %s -u %s -p %s -P %s  -h %s -o %s -T %s -r %s -D -t %s %s %s %s %s %s %s" % (dbname,username,password,port,dbhost,outputdir,back_table,rows,threads,kill_long_queries,snapshot_interval,no_locks,lock_all_tables,verbose,logfile))
if  import_table_struc:
    os.system("mysqldump -h %s -p%s -P %s -u %s -d %s %s > %s" %(dbhost,password,port,username,dbname,table_struc,table_struc_dir))
else:
    pass
