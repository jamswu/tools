...
# MySQL details
MYSQL_ACCESS="/usr/local/shell/mysql.conf"
...
get_from_status(){
  param=$1
  value=$(echo "show global status like '$param'" | $MYSQL  | awk '{print $2}'|grep -v Value)
  [ -z "$value" ] && echo $ERROR_WRONG_PARAM || echo $value
}

get_from_variables(){
  param=$1
  value=$(echo "show global variables like '$param'" | $MYSQL  | awk '{print $2}'|grep -v Value)
  [ -z "$value" ] && echo $ERROR_WRONG_PARAM || echo $value
}
...
get_from_slave(){
  param=$1
  value=$(echo "show slave status \G" | $MYSQL_SLAVE | grep -E "^[ ]*$param:" |grep -v Warning| awk '{print $2}')
  [ -z "$value" ] && echo $ERROR_WRONG_PARAM || echo $value
}

get_from_innodb_file(){
  param=$1
  datadir=$($MYSQL  --silent -e "show global variables like 'datadir';" | awk '{print $2}')
